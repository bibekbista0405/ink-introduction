...
export function PageTransition({ children }: { children: React.ReactNode }) {
  ...
  // Note: intentionally animates only opacity/transform (no `filter: blur()`).
  // Blur is one of the most expensive CSS properties for the browser to
  // rasterize every frame, and here it was stacked across the content
  // wrapper AND every staggered child on every single route change - the
  // single biggest source of navigation jank on mid-range/mobile devices.
  // Transform + opacity are compositor-only and stay smooth at 60fps.
